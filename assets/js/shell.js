...
export async function bootPage(onReady) {
  try {
    ...
    if (typeof onReady === "function") await onReady(state);
    initializeWeather().catch(error => log.warn("CLIMA", "Inicialização do clima não interrompeu o sistema.", error));
    log.success("INICIALIZAÇÃO", `${document.body.dataset.pageLabel || "Página"} pronta.`);
  } catch (error) {
    showFatal(error);
  }
}
