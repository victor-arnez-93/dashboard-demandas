...
export function renderReport() {
  ...
  body.innerHTML = filtered.length ? filtered.map(item => `<tr>
    <td>${demandCode(item)}</td><td>${escapeHtml(item.title)}</td><td>${escapeHtml(item.responsible)}</td>
  ...
  </tr>`).join("") : `<tr><td colspan="8" class="empty-table">Nenhuma demanda encontrada para os filtros.</td></tr>`;
}
...
export function exportExcel() {
  ...
  const rows = [["Código", "Título", "Descrição", "Solicitante", "Responsável", "Departamento", "Categoria", "Prioridade", "Status", "Entrada", "Prazo", "Horas estimadas", "Horas realizadas", "Tags", "Observações"]];
  filtered.forEach(item => rows.push([
    demandCode(item), item.title, item.description, item.requester || "", item.responsible, item.department || "", item.category,
  ...
  wsDemands["!autofilter"] = { ref: `A1:O${rows.length}` };
  wsDemands["!cols"] = [{ wch: 12 }, { wch: 34 }, { wch: 55 }, { wch: 22 }, { wch: 22 }, { wch: 18 }, { wch: 18 }, { wch: 12 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 15 }, { wch: 15 }, { wch: 28 }, { wch: 45 }];
...
export function exportPdf() {
  ...
  const done = filtered.filter(item => effectiveStatus(item) === "Concluída").length;
  const overdue = filtered.filter(item => effectiveStatus(item) === "Atrasada").length;
  const logo = new URL("assets/img/logo.jpeg", location.href).href;
  const rows = filtered.map(item => `<tr><td>${demandCode(item)}</td><td>${escapeHtml(item.title)}</td><td>${escapeHtml(item.responsible)}</td><td>${escapeHtml(item.category)}</td><td>${escapeHtml(item.priority)}</td><td>${escapeHtml(effectiveStatus(item))}</td><td>${formatDate(item.due_date, { year: true })}</td></tr>`).join("");
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Relatório de Demandas</title><style>
    @page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{font:11px Arial;color:#172027;margin:0}.head{text-align:center;border-bottom:3px solid #F96900;padding-bottom:12px}.head img{width:170px;height:54px;object-fit:contain;background:#050505;border-radius:12px}.head h1{margin:8px 0 2px;color:#284B63}.head p{margin:0;color:#68757c}.cards{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0}.card{border:1px solid #cdd4d7;border-radius:8px;padding:10px}.card small{display:block;color:#68757c}.card strong{font-size:20px;color:#284B63}table{width:100%;border-collapse:collapse}th{background:#284B63;color:#fff;text-align:left;padding:7px}td{border:1px solid #dce1e3;padding:6px;vertical-align:top}tr:nth-child(even){background:#f6f3eb}.foot{margin-top:12px;color:#68757c;font-size:9px;text-align:right}
  </style></head><body><div class="head"><img src="${logo}"><h1>${escapeHtml(state.settings.app_name)} — Relatório de Demandas</h1><p>${escapeHtml(document.getElementById("reportPeriodLabel").textContent)} · Gerado em ${new Date().toLocaleString("pt-BR")}</p></div><div class="cards"><div class="card"><small>Total</small><strong>${filtered.length}</strong></div><div class="card"><small>Concluídas</small><strong>${done}</strong></div><div class="card"><small>Em andamento</small><strong>${filtered.filter(item => effectiveStatus(item) === "Em andamento").length}</strong></div><div class="card"><small>Atrasadas</small><strong>${overdue}</strong></div></div><table><thead><tr><th>Código</th><th>Demanda</th><th>Responsável</th><th>Categoria</th><th>Prioridade</th><th>Status</th><th>Prazo</th></tr></thead><tbody>${rows}</tbody></table><div class="foot">FLUUX · Organização de Demandas</div><script>addEventListener("load",()=>setTimeout(()=>print(),250))<\/script></body></html>`;
...
export function initReports() {
  ...
  ["reportStatus", "reportPriority", "reportCategory", "reportResponsible"].forEach(id => document.getElementById(id).addEventListener(id === "reportResponsible" ? "input" : "change", renderReport));
